...
! JT - you should go through the list of unused variables that the compiler warns about and remove them
! to make it as lean / clear as possible
...
module base_state_module
  ...
  use meth_params_module, only: nscal, model_file, spherical, base_cutoff_density, &
                                ...
                                do_smallscale ! small scale doesnt appear to exist
  use base_state_geometry_module, only: nr_fine, dr, nr, max_radial_level

  use probin_module, only: p0_base, rho_1
...
contains
  ...
  subroutine init_base_state(s0_init,p0_init,rho0,rhoh0,p0,tempbar,tempbar_init) &
    ...
    double precision :: t_guess = 1.e-8

    if (spherical .eq. 1) then                      
       call amrex_error("ERROR: circular_drop base_state is not valid for spherical")
    endif

    ! might want to add the density cut off warnings as per e.g. rt here
      ...
      ! height above the bottom of the domain
        ...
        if (do_smallscale) then
           p_ambient = p0_base
            ...
            p_ambient = p0_base + rho_1*grav_const*(rloc - prob_lo(amrex_spacedim))
        end if
        t_ambient = t_guess
...
! I don't know what this subroutine does so ive stubbed it for now
