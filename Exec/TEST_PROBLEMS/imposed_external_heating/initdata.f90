...
contains
  ...
  subroutine initdata(lev, time, lo, hi, &
    ...
    ! 
    double precision :: upert(1:nc_v)
    double precision :: fwtm ! full width tenth maximum of gaussian perturbation 


    fwtm = scale_height / 5d0
...
!    call amrex_error()
    ...
    do i=lo(1),hi(1)
      ...
      upert(1) = ZERO
      upert(2) = pert_amp * exp(-(y-scale_height/2)**2 / 2d0 / (fwtm / 4.29193d0)**2 ) 
      !2d only atm, no upert(3)
