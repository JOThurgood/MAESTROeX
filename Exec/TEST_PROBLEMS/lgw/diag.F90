...
!
! This version of diag is the local override for the lgw problem
! It builds on the version of diag.F90 copied over from `Src_nd` on 8 oct
...
! entropy like terms of linear wave action instead.
  ...
  ! for action experiment 
  use probin_module, only: diag_action, pres_base, scale_height
...
contains
  ...
  subroutine diag(lev, lo, hi, &
    ...
    ! Local variables for action experiment
    ...
    ! experimental action stuff
    ! overwrite kin_ener and int_ener with the linear wave action quantitites
    if (diag_action) then
      kin_ener = action_u
      int_ener = action_s
    endif
