...
! Before our changes to diag for planar geometries were merged into the main code,
! we were overriding it by placing a copy of `MAESTROeX/Source/Src_nd/diag.F90` 
! (and also the C++ code) in the problem directory.
! 
! When we considered conservation of linear wave action, the fastest way to have a look was to 
! just hack that code to record the relevant conserved quantities of action in place of kinetic
! energy (replace with the momentum related term) and internal energy (replace the entropy
! related term).
! 
! To repeat this, there would be no need to override the C++ anymore, but you'd need to bring
! `diag.F90` over and modify the quantity you are integrating
! 
! This is the code I was using last time.
! The action stuff is commented out so that would have to go back in. 
! Also be careful to check it doesn't confict with any
! changes to diag.f90 that might have happened in the development branch since

! I actually recommend copying the current `MAESTROeX/Source/Src_nd/diag.F90` to the problem 
! directory, then making such changes to that, to ensure its reasonabally up to date.

! i.e., this is for reference in case it is useful
